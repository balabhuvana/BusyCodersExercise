...
class ViewHolderFragment : Fragment() {
    ...
    inner class IconicAdapter : ArrayAdapter<String>(this@ViewHolderFragment.activity, R.layout.custom_array_adapter_row, R.id.bigtext, items) {
        override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {

            val row = super.getView(position, convertView, parent)
            var holder: ViewHolder? = row.tag as? ViewHolder
            ...
            if (items[position].length > 4) holder.sampleImageView!!.setImageResource(R.drawable.ok) else holder.sampleImageView!!.setImageResource(R.drawable.delete)
